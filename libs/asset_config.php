 ...
 */
    public static function buildFromIniFile($iniFile = null, $constants = array()){
        ...
        // Allows us to override the CONFIGS value specifically for testing.
        if (!empty($constants) && isset($constants['CONFIGS/'])) {
            $configsPath = $constants['CONFIGS/'];
        } else {
            $configsPath = CONFIGS;
        }

        if (empty($iniFile)) {
            $iniFile = $configsPath . 'asset_compress.ini';
        }

        // If the asset_compress dir exists and we have env specific config files, use the
        // appropriate one.
        $env = Configure::read('environment');
        $envFile = $configsPath . 'asset_compress' . DS . $env . '.ini';
        if (!is_null($env) && file_exists($envFile)) {
            $iniFile = $envFile;
        }
